...
#include "frame_equalizer_tap_impl.h"
#include "utils.h"
...
frame_equalizer_tap::sptr
frame_equalizer_tap::make(Equalizer algo, double freq, double bw, bool log, bool debug)
{
    return gnuradio::get_initial_sptr(
        new frame_equalizer_tap_impl(algo, freq, bw, log, debug));
}


frame_equalizer_tap_impl::frame_equalizer_tap_impl(
    Equalizer algo, double freq, double bw, bool log, bool debug)
    : gr::block("frame_equalizer_tap",
...
void frame_equalizer_tap_impl::set_algorithm(Equalizer algo)
{
...
void frame_equalizer_tap_impl::set_bandwidth(double bw)
{
    gr::thread::scoped_lock lock(d_mutex);
    d_bw = bw;
}

void frame_equalizer_tap_impl::set_frequency(double freq)
{
    gr::thread::scoped_lock lock(d_mutex);
    d_freq = freq;
}

void frame_equalizer_tap_impl::forecast(int noutput_items,
                                        gr_vector_int& ninput_items_required)
{
    ninput_items_required[0] = noutput_items;
}

int frame_equalizer_tap_impl::general_work(int noutput_items,
...
bool frame_equalizer_tap_impl::decode_signal_field(uint8_t* rx_bits)
{
...
void frame_equalizer_tap_impl::deinterleave(uint8_t* rx_bits)
{
    for (int i = 0; i < 48; i++) {
...
bool frame_equalizer_tap_impl::parse_signal(uint8_t* decoded_bits)
{
...
const int frame_equalizer_tap_impl::interleaver_pattern[48] = {
